    ...
    hint="I provide a way to serialize complex ColdFusion data values as case-sensitive JavaScript Object Notation (JSON) strings."
{
    ...
    // Added addition dynamic call for the above functions
    public any function asType(required string type, required string key) {
        switch (lcase(arguments.type)) {
            case 'boolean': return asBoolean(arguments.key); break;
            case 'date': return asDate(arguments.key); break;
            case 'float': return asFloat(arguments.key); break;
            case 'integer': return asInteger(arguments.key); break;
            case 'string': return asString(arguments.key); break;
            case 'any': return asAny(arguments.key); break;
            default: throw(
                type = 'InvalidType',
                message = 'The type [#type#] is not a valid type.',
                detail = 'The current types available are: boolean,date,float,integer,string,any'
            );
        }
    }
    ...
    private void function serializeInput(
        ...
        if (isSimpleValue(input)) {
            if ((hint == 'string') || (hint == 'any')) {
                ...
                ((hint == 'integer') || (hint == 'float')) && isNumeric(input)
            ) {
                writeOutput(input);
            } else if (
                ((hint == 'integer') || (hint == 'float')) && isBoolean(input)
            ) {
                ...
                // TODO: Allow overriding of the date format
                writeOutput(
                    '"' & dateFormat(input, 'yyyy-mm-dd') & 'T' & timeFormat(
                        input,
                        'HH:mm:ss.l'
                    ) & 'Z"'
                );
    ...
    private void function serializeInputString(required string input) {
        ...
        // a logic error
        arguments.input = javacast('string', arguments.input);

        var length = arguments.input.length();

        writeOutput('"');

        for (var i = 1; i <= length; i++) {
            var charCode = arguments.input.codePointAt(javacast('int', i - 1));
            ...
            if (
                ...
                writeOutput(
                    '\u' & right(('000' & formatBaseN(charCode, 16)), 4)
                );
            } else if (charCode == 34) {
                writeOutput('\"');
            } else if (charCode == 47) {
